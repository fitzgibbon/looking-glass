...
(ert-deftest lg-effectful-wrapper-coverage ()
  (let* ((list-source '(1 2 3))
         (index-source [10 20 30])
         (review-optic (lg-iso #'number-to-string #'string-to-number)))
    (should (equal (lg-run-effect (lg-effect-to-list-of lg-list) list-source)
                   (lg-to-list-of lg-list list-source)))
    (should (equal (lg-run-effect (lg-effect-foldl-of lg-list #'+ 0) list-source)
                   (lg-foldl-of lg-list #'+ 0 list-source)))
    (should (equal (lg-run-effect (lg-effect-any-of lg-list (lambda (x) (= x 2))) list-source)
                   (lg-any-of lg-list (lambda (x) (= x 2)) list-source)))
    (should (equal (lg-run-effect (lg-effect-maximum-of lg-list) list-source)
                   (lg-maximum-of lg-list list-source)))
    (should (equal (lg-run-effect (lg-effect-ipreview-of lg-indexed-vector) index-source)
                   (lg-ipreview lg-indexed-vector index-source)))
    (should (equal (lg-run-effect (lg-effect-ito-list-of lg-indexed-vector) index-source)
                   (lg-ito-list-of lg-indexed-vector index-source)))
    (should (equal (lg-run-effect (lg-effect-ifoldl-of lg-indexed-vector
                                                        (lambda (acc i x) (+ acc (* i x)))
                                                        0)
                                  index-source)
                   (lg-ifoldl-of lg-indexed-vector
                                 (lambda (acc i x) (+ acc (* i x)))
                                 0
                                 index-source)))
    (should (equal (lg-review! review-optic "42") 42))
    (should (equal (lg-reviews! review-optic (lambda (x) (* x 2)) "21") 42))
    (should (equal (lg-foldr-of! lg-list (lambda (x acc) (+ x acc)) 0 list-source)
                   (lg-foldr-of lg-list (lambda (x acc) (+ x acc)) 0 list-source)))
    (should (equal (lg-preview-or! :missing (lg-plist-key :name) '(:age 10))
                   (lg-preview-or :missing (lg-plist-key :name) '(:age 10))))
    (should (equal (lg-ipreview-or! :missing lg-indexed-vector [])
                   (lg-ipreview-or :missing lg-indexed-vector [])))
    (should (equal (lg-iset! lg-indexed-vector 0 [1 2 3])
                   (lg-iset lg-indexed-vector 0 [1 2 3])))
    (should (equal (lg-map-of! lg-list (lambda (x) (+ x 1)) list-source)
                   (lg-map-of lg-list (lambda (x) (+ x 1)) list-source)))
    (should (equal (lg-ifor-of! lg-indexed-vector index-source
                                (lambda (i x) (+ x i)))
                   (lg-ifor-of lg-indexed-vector index-source
                               (lambda (i x) (+ x i)))))))
