...
(ert-deftest lg-compile-unindexed-runners ()
  (let* ((optic (lg-compose (lg-lens #'car (lambda (source focus)
                                         (cons focus (cdr source))))
                            (lg-lens #'car (lambda (source focus)
                                             (cons focus (cdr source))))))
         (source '((1 2) 3)))
    (should (equal (funcall (lg-compile optic 'over #'1+) source) '((2 2) 3)))
    (should (equal (funcall (lg-compile optic 'set 9) source) '((9 2) 3)))
    (should (equal (funcall (lg-compile optic 'to-list) source) '(1)))
    (should (equal (funcall (lg-compile optic 'preview) source) (lg-just 1)))
    (should (= (funcall (lg-compile optic 'view) source) 1))
    (should (funcall (lg-compile optic 'has) source))
    (should (= (funcall (lg-compile optic 'fold-map lg-monoid-sum #'identity)
                        source)
               1)))
  (should (equal (funcall (lg-compile lg-list 'over #'1+) '(1 2 3))
                 '(2 3 4)))
  (should (= (funcall (lg-compile (lg-iso #'number-to-string #'string-to-number)
                                  'over
                                  (lambda (text) (concat text "0")))
                       42)
             420))
  (let ((missing (lg-compile lg-non-nil 'preview)))
    (should (equal (funcall missing nil) lg-nothing)))
  (should-error (funcall (lg-compile lg-non-nil 'view) nil))
  (should-error (lg-compile lg-car 'over))
  (should-error (lg-compile lg-car 'review)))

(ert-deftest lg-compile-indexed-runners ()
  (let ((source '(1 2 3)))
    (should (equal (funcall (lg-compile lg-indexed-list 'iover
                                        (lambda (index focus) (+ index focus)))
                            source)
                   '(1 3 5)))
    (should (equal (funcall (lg-compile lg-indexed-list 'iset 9) source)
                   '(9 9 9)))
    (should (equal (funcall (lg-compile lg-indexed-list 'ito-list) source)
                   '((0 . 1) (1 . 2) (2 . 3))))
    (should (equal (funcall (lg-compile lg-indexed-list 'ipreview) source)
                   (lg-just '(0 . 1))))
    (should (equal (funcall (lg-compile lg-indexed-list 'iview) source)
                   '(0 . 1)))
    (should (funcall (lg-compile lg-indexed-list 'ihas) source))
    (should (= (funcall (lg-compile lg-indexed-list 'ifold-map
                                    lg-monoid-sum
                                    (lambda (index focus) (* index focus)))
                        source)
               8))))

(ert-deftest lg-compile-interprets-custom-optics-once ()
  (let* ((runs 0)
         (optic (make-lg-optic
                :apply (lambda (_profunctor pab)
                         (setq runs (1+ runs))
                         pab))))
    (let ((compiled (lg-compile optic 'over #'1+)))
      (should (= runs 1))
      (should (= (funcall compiled 1) 2))
      (should (= (funcall compiled 2) 3))
      (should (= runs 1)))))
