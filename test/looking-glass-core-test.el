...
(ert-deftest lg-dual-cochoice-costrong-function-and-forget ()
  (let* ((fn-p (lg--function-profunctor))
         (plus1 (lambda (x) (+ x 1)))
         (first-plus1 (funcall (lg-profunctor-first fn-p) plus1))
         (right-plus1 (funcall (lg-profunctor-right fn-p) plus1))
         (unfirst-plus1 (funcall (lg-profunctor-unfirst fn-p) first-plus1))
         (unright-plus1 (funcall (lg-profunctor-unright fn-p) right-plus1))
         (forget-p (lg--forget-profunctor nil))
         (forget-right (make-lg-forget
                        :run (lambda (either)
                               (if (lg-right-p either)
                                   (list (cdr either))
                                 nil))))
         (forget-unright (funcall (lg-profunctor-unright forget-p) forget-right)))
    (should (= (funcall unfirst-plus1 10) 11))
    (should (= (funcall unright-plus1 10) 11))
    (should (equal (funcall (lg-forget-run forget-unright) 7) '(7)))))
...
(ert-deftest lg-map-for-and-hash-key-aliases ()
  (should (equal (lg-map-of lg-list #'1+ '(1 2)) '(2 3)))
  (should (equal (lg-for-of lg-list '(1 2) #'1+) '(2 3)))
  (should (equal (lg-imap-of lg-indexed-list (lambda (i x) (+ i x)) '(10 10))
                 '(10 11)))
  (should (equal (lg-ifor-of lg-indexed-list '(10 10) (lambda (i x) (+ i x)))
                 '(10 11)))
  (let ((table (make-hash-table :test 'equal)))
    (puthash "k" 1 table)
    (should (equal (lg-preview (lg-hash-key "k") table) (lg-just 1)))
    (should (equal (lg-preview (lg-hash-key "missing") table) lg-nothing))
    (should (= (gethash "k" (lg-set (lg-hash-key-at "k") (lg-just 2) table)) 2))
