...
- Macro sugar: =lg/path-json=, =lg^=, =lg==, =lg~=, =lg~i=, =lg->=,
  =lg->>=.
...
;; Macro sugar
(lg~ json
     (lg-compose (lg-json-string-alist)
                 (lg/path-json "user" "name"))
     #'upcase)
#+end_src

* Buffer extension

This repo also includes =looking-glass-buffer.el= for buffer optics:

- Content optics: =lg-buffer-string=, =lg-buffer-region=,
  =lg-buffer-lines=, =lg-buffer-matches=.
- Metadata optics: =lg-buffer-name=, =lg-buffer-modified-p=,
  =lg-buffer-point=, =lg-buffer-mark=.
- Indexed traversals: =lg-ibuffer-lines=, =lg-ibuffer-matches=.
...
This repo also includes =looking-glass-tree-sitter.el= for AST-aware optics in
tree-sitter-enabled buffers:

- Root/query node access: =lg-ts-root-node=, =lg-ts-query-nodes=,
  =lg-its-query-nodes=.
- Query text traversals for structural rewrites: =lg-ts-query=,
  =lg-its-query=.

Example (increment all JSON number nodes in current buffer):
...
(lg-over (lg-ts-query '((number) @num) 'json 'num)
         (lambda (s) (number-to-string (1+ (string-to-number s))))
         (current-buffer))
#+end_src
